...
// Component for adding admin drawer
const AddAdminDrawer = ({
  ...
  // Handle adding multiple admins from CSV
  const addMultipleAdminHandler = async () => {
    setLoading(false);
    if (csvfile) {
      try {
        setLoading(true);
        const res = await axios({
          method: "POST",
          url: ApiConfig.addMultipleSubAdmin,
          headers: {
  ...
  const addAdminhandler = async (values) => {
    ...
    try {
      ...
      const res = await axios({
        method: "POST",
        url: ApiConfig.adminCreateSubAdmin,
        headers: {
          token: localStorage.getItem("token"),
        },
        data: {
  ...
  // Handle click to switch to single project form
  const handleSingleProjectClick = () => {
    setIsSingleProject(true);
  };
  // Handle click to switch to multiple project form
                ...
                <Box className={classes.csvContainer}>
                  ...
                  <Box className={classes.saveDeailsBtnContainer}>
                    <Button
                      variant="contained"
                      className={classes.saveDetailsBtn}
                      onClick={addMultipleAdminHandler}
                    >
